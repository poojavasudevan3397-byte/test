...
#         try:
#             # Query batting_stats table for comprehensive batting stats
#             query = "SELECT player_id, player_name, runs_scored, innings_played, batting_average, strike_rate FROM batting_stats ORDER BY"
...
#             st.info(f"📊 Total records in database: {len(_safe_read_sql('SELECT COUNT(*) as cnt FROM batting_stats', conn))}")
...
#         try:
#             # Query bowling_stats table
#             query = "SELECT player_id, player_name, wickets_taken, bowling_average, economy_rate FROM bowling_stats ORDER BY"
...
#             st.info(f"📊 Total records in database: {len(_safe_read_sql('SELECT COUNT(*) as cnt FROM bowling_stats', conn))}")
...
#         with col1:
#             batting_count = _safe_read_sql("SELECT COUNT(*) as cnt FROM batting_stats", conn).iloc[0, 0]
#             st.metric("Total Batsmen", f"{batting_count:,}")
        
#         with col2:
#             bowling_count = _safe_read_sql("SELECT COUNT(*) as cnt FROM bowling_stats", conn).iloc[0, 0]
